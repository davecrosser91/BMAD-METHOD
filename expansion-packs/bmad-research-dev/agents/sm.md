...
github-helper-scripts:
  ...
  available-scripts:
    create-issue:
      ...
      when-to-use:
        - Creating ad-hoc story issues
        - Quick issue creation for tasks
      options: '--label "type:story,size:m,priority:p2" --milestone "Epic 1" --project 1'
      example: |
        # Create story issue
        {root}/scripts/create-issue.sh \
          --title "Add user profile page" \
          --body "User story details..." \
          --label "type:story,size:m,priority:p2" \
          --milestone "Epic 1" \
          --project 1
    ...
    search-issues:
      ...
      when-to-use:
        - Managing backlog
        - Sprint planning
        - Finding stories by epic/milestone
      example: |
        # Find all backlog stories for Epic 1
        {root}/scripts/search-issues.sh --label "type:story" --milestone "Epic 1" --state open

    get-issue-details:
      command: '{root}/scripts/get-issue-details.sh {issue-number} [--format json|yaml|text]'
      when-to-use:
        - Reviewing story details
